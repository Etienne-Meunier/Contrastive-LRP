...
def flatten_forward(self, input, output) :
    self.shape_before = input[0].shape[1:]

def flatten_backward(self, grad_input, grad_output) :
     global R
     R = R.reshape((-1, *self.shape_before))
     #print(f'test flbh end nan : {torch.isnan(R).sum()}')
...
def conv_backward_hook(self, grad_input, grad_output):
    ...
    #print(f'test cbh end nan : {torch.isnan(R).sum()}')
...
def backmaxpoolhook_wta(self, grad_input, grad_output):
    global R

    with torch.enable_grad():
        Input = Variable(self.input.data, requires_grad=True)
        Out = F.max_pool2d(Input, kernel_size=self.kernel_size,
                       stride=self.stride, padding=self.padding, ceil_mode=self.ceil_mode)
        Out.backward(R)
        R = Input.grad


def maxPool_relprop(input, size=3, stride=1, padding=0, R_temp = None):
    out = F.max_pool2d(input, kernel_size=size, stride=stride, padding=padding, ceil_mode=True)
...
def maxpool_backward_hook(self, grad_input, grad_output):
    global R
    #print(f'test mbh nan : {torch.isnan(R).sum()}')
    ...
    #print(f'test mbh end nan : {torch.isnan(R).sum()}')
